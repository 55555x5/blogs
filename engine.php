<?php include "cache/".strtolower($_GET["b"]).".php";
if($custom_css!=False){$custom_css="<link rel='stylesheet' href='$custom_css'>";}else{$custom_css="";}
...
$footer="</div><footer>pwd by <a href='https://bl.7ort.rf.gd'>7ort Blogs</a></footer></body></html>";
if(isset($_GET["p"])){
    function notfound($errno, $errstr){die("Error!\nPost not found");}
    set_error_handler("notfound");
    header("Content-type: text/html");
    $post=$_GET["p"]-1;
    echo "<!DOCTYPE html><html><head><!--~weblogs v1.1.1--><meta name='viewport' content='width=device-width,initial-scale=1'><title>".$posts[$post][1]." | $title</title><style>$style</style>$custom_css</head><body><a id='logo' href='?b=".$_GET["b"]."'><h1>$title</h1></a><div class='notlogo'>";
...
<?php }else{
    ...
    echo "<!DOCTYPE html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>$title</title><style>$style</style>$custom_css</head><body><a id='logo' href='?b=".$_GET["b"]."'><h1>$title</h1></a>$description$website<a href='?b=".strtolower($_GET["b"])."&rss'>RSS Feed</a><br><div class='notlogo'>";
